...
print("⏳ Extracting Balanced Context Sentences from Kaggle...")
...
HARD_NEGATIVES = [
    "2021", "2020", "2015", "1999", "mapping", "mind mapping", "story", "challenge", "handbook", 
    "insights", "john stuart mill", "fehr", "ythier", "ernst", "student", "teacher", "course",
    "design thinking", "visualization", "learning launch"
]
...
for doc in kaggle_data:
    ...
    for hn in HARD_NEGATIVES:
        if hn.lower() in text.lower():
            idx = text.lower().find(hn.lower())
            context = text[max(0, idx-80): min(len(text), idx+len(hn)+80)]
            negatives.append(f"{hn} [SEP] {context}")
            
    # Extract Random "O" tokens that look like names/nouns (Capitalized) to teach it not all caps are PII
    for token, label in zip(doc["tokens"], doc["labels"]):
        if label == "O" and token.istitle() and len(token) > 3 and token.lower() not in HARD_NEGATIVES:
            if random.random() < 0.05:  # Sample 5% of them
                idx = text.find(token)
                if idx != -1:
                    context = text[max(0, idx-80): min(len(text), idx+len(token)+80)]
                    negatives.append(f"{token} [SEP] {context}")

# Balance the dataset (Match the sizes)
min_size = min(len(positives), len(negatives), 500) # Cap at 500 each for fast training
...
print("🚀 Training Balanced Binary Sequence Rejector...")
trainer.train()
